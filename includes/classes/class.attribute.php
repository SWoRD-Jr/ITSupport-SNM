...
class Attribute extends App {
    ...
    public static function addWebServiceCategory($data) {
        global $database;
        if (!isset($data['color']))
            $data['color'] = "#167cc1";
        $lastid = $database->insert("webservicecategories", ["name" => $data['name'], "color" => $data['color']]);
        if ($lastid == "0") {
            return "11";
        } else {
            logSystem("Web Service Category Added - ID: " . $lastid);
            return "10";
        }
    }

    public static function editWebServiceCategory($data) {
        global $database;
        if (!isset($data['color']))
            $data['color'] = "#167cc1";
        $database->update("webservicecategories", ["name" => $data['name'], "color" => $data['color']], ["id" => $data['id']]);
        logSystem("Web Service Category Edited - ID: " . $data['id']);
        return "20";
    }

    public static function deleteWebServiceCategory($id) {
        global $database;
        $database->delete("webservicecategories", ["id" => $id]);
        logSystem("Web Service Category Deleted - ID: " . $id);
        return "30";
    }

    
    // ----------------------------------------------------------------------------------------------
    // WEB SERVICE ACCOUNT NUMBER (godaddy/rackspace...etc)

    public static function addAccountNumber($data) {
        global $database;
        
        $lastid = $database->insert("accountnumbers", ["accountnumber" => $data['accountnumber'], "accountname" => $data['accountname'], "supplierid" => $data['supplierid']]);
        if ($lastid == "0") {
            return "11";
        } else {
            logSystem("WS Account Added - ID: " . $lastid);
            return "10";
        }
    }

    public static function editAccountNumber($data) {
        global $database;
        
        $database->update("accountnumbers", ["accountnumber" => $data['accountnumber'], "accountname" => $data['accountname'], "supplierid" => $data['supplierid']], ["id" => $data['id']]);
        logSystem("WS Account Edited - ID: " . $data['id']);
        return "20";
    }

    public static function deleteAccountNumber($id) {
        global $database;
        $database->delete("accountnumbers", ["id" => $id]);
        logSystem("WS Account Deleted - ID: " . $id);
        return "30";
    }    
