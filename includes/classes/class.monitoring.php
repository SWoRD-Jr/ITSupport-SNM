...
class Monitoring extends App {
    ...
    public static function addHost($data) {
        ...
        if ($lastid == "0") {
            ...
            logSystem("Host Added - ID: " . $lastid);
            return "10";
        }
    }
    ...
    public static function addCheck($data) {
        ...
        if ($lastid == "0") {
            ...
            logSystem("Check Added - ID: " . $lastid);
            return "10";
        }
    }
    ...
    public static function alertDown($checkid) { //send alert if state changes to down
        ...
        if ($check['status'] != "Down") {
            ...
            foreach ($asignedPeople as $assigned) {
                if ($check['email'] == 1)
                    Notification::monitoringEmail($assigned['peopleid'], $check['hostid'], $hostinfo, $status);
                if ($check['sms'] == 1)
                    Notification::monitoringSMS($assigned['peopleid'], $check['hostid'], $hostinfo, $status);
            }
        }
    }
        ...
        foreach ($checks as $check) {
            ...
            if ($check['monitoring'] == 1 && $check['type'] == "Service") {
                ...
                if ($result == 0) {
                    ...
                    if ($recheck == 0) {
                        self::addHistory($check['id'], "Down", $recheck);
                        self::alertDown($check['id']);
                    } else {
                        self::addHistory($check['id'], "Up", $recheck);
